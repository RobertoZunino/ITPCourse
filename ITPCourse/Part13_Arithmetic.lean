...
example (a b c: ℕ)
  ...
  -- We move to integers
  rw [← Nat.cast_inj (R:=ℤ)]
  -- Or, alternatively: `apply (Nat.cast_inj (R:=ℤ)).mp`.
  --
...
/-
  We now solve a simple quadratic equation using factoring.
-/
example
  (x: ℝ)
  (eq: x^2 - 6*x + 5 = 0)
  ...
  := by
  -- Manual factoring
  convert_to (x - 1) * (x - 5) = 0 at eq
  . ring
  -- `a * b = 0` iff `a` or `b` is zero
  rw [mul_eq_zero] at eq
  grind -- Or, alternatively, basic case analysis

/-
  We solve the same quadratic equation using the quadratic formula.

  We first compute the discriminant and take its root.
-/
#eval discrim 1 (-6) 5    -- 16, i.e. 4^2
...
example
  ...
  : x = 1 ∨ x = 5
  := by
  -- Bring the quadratic equation into its normal form
  convert_to (1*(x*x) + (- 6)*x + 5 = 0) at eq
  . ring
  -- Apply the quadratic formula (`s` = root of the discriminant)
  rw [quadratic_eq_zero_iff (s := 4)] at eq
  -- A few subgoals generated by `rw`
  case ha =>
    -- The equation is indeed quadratic, i.e., its leading coefficient
    --  is non zero.
    linarith
  case h =>
    -- We prove that `s` is indeed the root of `discrim`
    unfold discrim
    ring
